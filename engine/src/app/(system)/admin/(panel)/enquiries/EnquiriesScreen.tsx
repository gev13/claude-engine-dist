...
export function EnquiriesScreen({ canChange = false }: { canChange?: boolean }) {
  return (
    <ToastProvider>
      <EnquiriesScreenInner canChange={canChange} />
    </ToastProvider>
  );
}

function EnquiriesScreenInner({ canChange }: { canChange: boolean }) {
  ...
  return (
    <>
      ...
      {/* An enquiry is somebody's name, address and message, kept only as long
          as the conversation it started might reopen. */}
      <div className="mt-6">
        <RetentionPanel
          endpoint="/api/admin/enquiries"
          retention={data?.retention}
          canChange={canChange}
          noun="contact enquiries"
          alsoDeletes="and its message"
          onSaved={() => mutate()}
        />
      </div>
    </>
