...
export function InstallWizard() {
  ...
  const [site, setSite] = useState({
    ...
    /** The site's main language. More are added later, in Settings. */
    locale: 'en',
  });

  /** Set when the site is installed but the chosen language needs a restart. */
  const [installedNeedsRestart, setInstalledNeedsRestart] = useState<string | null>(null);
  ...
  async function install() {
    ...
    setBusy(true);
    try {
      const result = await api<{ redirectTo: string; restartRequired?: boolean }>('/api/install', {
        method: 'POST',
        json: {
          admin: {
          ...
          site: { name: site.name, tagline: site.tagline, timeZone: site.timeZone, locale: site.locale },
        },
      });

      /* A language other than the default was written to .env, which this
         process read at boot. Sending them to the admin now would show a site
         still rendering in the old language, with nothing to explain it. */
      if (result.restartRequired) {
        setInstalledNeedsRestart(result.redirectTo);
        setBusy(false);
        return;
      }
  ...
  /* Installed, but rendering in the wrong language until the process restarts:
     the language lives in .env, which was read at boot. */
  if (installedNeedsRestart) {
    return (
      <div className="space-y-5">
        <Alert tone="info">
          Installed. Your account is ready and the site is set up in {LANGUAGES[site.locale]?.name ?? site.locale}.
        </Alert>
        <p className="m-0 text-[15px] leading-relaxed text-ash">
          <strong className="text-bone">Restart the site now.</strong> The language was written to{' '}
          <code className="font-mono text-flare-soft">.env</code>, and this process read its settings when it started —
          until it restarts, the site still renders in English. Under pm2 that is{' '}
          <code className="font-mono text-flare-soft">pm2 restart engine</code>; otherwise stop it and run{' '}
          <code className="font-mono text-flare-soft">npm start</code> again.
        </p>
        <p className="m-0 text-[15px] leading-relaxed text-ash">
          Then <Link href={installedNeedsRestart} className="underline">carry on to the admin panel</Link>.
        </p>
      </div>
    );
  }
      ...
      {step === 2 && (
        <div className="space-y-5">
          ...
          <div className="grid gap-4">
            ...
            <Field
              label="Main language"
              hint="what the site is written in. More can be added later, in Settings → Languages."
            >
              <Select value={site.locale} onChange={(e) => setSite({ ...site, locale: e.target.value })}>
