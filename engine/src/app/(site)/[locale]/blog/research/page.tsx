...
export default async function ResearchIndex({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const [posts, settings, theme] = await Promise.all([listPosts({ kind: "research", limit: 48, locale }), getSiteSettings(), getTheme()]);
