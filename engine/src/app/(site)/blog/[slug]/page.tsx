...
import { allPublishedPostSlugs, getPost, listPosts } from '@/server/content/posts';
...
export async function generateStaticParams() {
  const slugs = await allPublishedPostSlugs();
...
export default async function PostPage({ params }: { params: Promise<Params> }) {
  ...
  const related = (await listPosts({ kind: post.kind, limit: 4 })).filter((p) => p.slug !== post.slug).slice(0, 3);
