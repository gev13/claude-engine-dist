            ...
            <>
              <Panel title="Palette">
                <div className="grid gap-4 sm:grid-cols-2">
                  <ColorField label="Page background" hint="the base surface" value={get(['colors', 'background'])} onChange={set(['colors', 'background'])} />
                  <ColorField label="Raised surface" hint="cards and alternate bands" value={get(['colors', 'surface'])} onChange={set(['colors', 'surface'])} />
                  <ColorField label="Third surface" hint="hover states" value={get(['colors', 'surfaceRaised'])} onChange={set(['colors', 'surfaceRaised'])} />
                  <ColorField label="Primary accent" value={get(['colors', 'primary'])} onChange={set(['colors', 'primary'])} />
                  <ColorField label="Accent hover" value={get(['colors', 'primaryHover'])} onChange={set(['colors', 'primaryHover'])} />
                  <ColorField label="Accent soft" hint="link hover, numerals" value={get(['colors', 'primarySoft'])} onChange={set(['colors', 'primarySoft'])} />
                </div>
              </Panel>

              <Panel title="Text and rules">
                <div className="grid gap-4 sm:grid-cols-2">
                  <ColorField label="Primary text" value={get(['colors', 'textPrimary'])} onChange={set(['colors', 'textPrimary'])} />
                  <ColorField label="Body text" value={get(['colors', 'textBody'])} onChange={set(['colors', 'textBody'])} />
                  <ColorField label="Muted text" hint="labels and meta" value={get(['colors', 'textMuted'])} onChange={set(['colors', 'textMuted'])} />
                  <ColorField label="Hairline" hint="section dividers" value={get(['colors', 'hairline'])} onChange={set(['colors', 'hairline'])} />
                  <ColorField label="Rule" hint="stronger borders" value={get(['colors', 'rule'])} onChange={set(['colors', 'rule'])} />
                  <ColorField label="Selection" hint="highlighted text" value={get(['colors', 'selection'])} onChange={set(['colors', 'selection'])} />
                  <ColorField label="Link" value={get(['colors', 'link'])} onChange={set(['colors', 'link'])} />
                  <ColorField label="Link hover" value={get(['colors', 'linkHover'])} onChange={set(['colors', 'linkHover'])} />
                </div>
          ...
          {tab === 'Typography' && (
            <>
              ...
              <Panel title={`${TYPE_ROLE_LABELS[role]} — base`}>
                <div className="grid gap-4 sm:grid-cols-2">
                  <ChoiceField label="Font family" value={get(['typography', role, 'base', 'family']) as FontKey | undefined} options={FONT_OPTIONS} onChange={set(['typography', role, 'base', 'family'])} />
                  <LengthField label="Font size" placeholder="e.g. 66px or clamp(36px, 7vw, 66px)" value={get(['typography', role, 'base', 'size'])} onChange={set(['typography', role, 'base', 'size'])} />
                  <ChoiceField label="Font weight" value={get(['typography', role, 'base', 'weight']) as never} options={WEIGHT_OPTIONS} onChange={set(['typography', role, 'base', 'weight'])} />
                  <ChoiceField label="Font style" value={get(['typography', role, 'base', 'style']) as never} options={STYLE_OPTIONS} onChange={set(['typography', role, 'base', 'style'])} />
                  <LengthField label="Line height" kind="lineHeight" placeholder="e.g. 1.2" value={get(['typography', role, 'base', 'lineHeight'])} onChange={set(['typography', role, 'base', 'lineHeight'])} />
                  <LengthField label="Letter spacing" placeholder="e.g. -0.03em" value={get(['typography', role, 'base', 'letterSpacing'])} onChange={set(['typography', role, 'base', 'letterSpacing'])} />
                  <ColorField label="Colour" value={get(['typography', role, 'base', 'color'])} onChange={set(['typography', role, 'base', 'color'])} />
                  <ChoiceField label="Text transform" value={get(['typography', role, 'base', 'transform']) as never} options={TRANSFORM_OPTIONS} onChange={set(['typography', role, 'base', 'transform'])} />
                  <ChoiceField label="Text decoration" value={get(['typography', role, 'base', 'decoration']) as never} options={DECORATION_OPTIONS} onChange={set(['typography', role, 'base', 'decoration'])} />
                </div>
              </Panel>

              <Panel title={`${TYPE_ROLE_LABELS[role]} — by screen size`}>
                <p className="m-0 mb-4 text-[13px] text-smoke">
                  Each row applies at that width and below, and overrides the base above. Leave a field empty to
                  inherit. A fluid <code className="font-mono text-flare-soft">clamp()</code> base size often removes the
                ...
                <div className="space-y-4">
                  {BREAKPOINTS.map((bp) => (
                    <div key={bp.key} className="grid gap-3 border-t-2 border-hairline pt-4 sm:grid-cols-3">
                      ...
                      <LengthField label="Font size" value={get(['typography', role, bp.key as BreakpointKey, 'size'])} onChange={set(['typography', role, bp.key, 'size'])} />
                      <LengthField label="Line height" kind="lineHeight" value={get(['typography', role, bp.key as BreakpointKey, 'lineHeight'])} onChange={set(['typography', role, bp.key, 'lineHeight'])} />
                      <LengthField label="Letter spacing" value={get(['typography', role, bp.key as BreakpointKey, 'letterSpacing'])} onChange={set(['typography', role, bp.key, 'letterSpacing'])} />
                    </div>
                  ))}
                </div>
              </Panel>
          ...
          {tab === 'Buttons' && (
            <>
              <Panel title="Shared geometry">
                <div className="grid gap-4 sm:grid-cols-2">
                  <LengthField label="Corner radius" placeholder="0px" value={get(['buttons', 'radius'])} onChange={set(['buttons', 'radius'])} />
                  <LengthField label="Font size" placeholder="12px" value={get(['buttons', 'fontSize'])} onChange={set(['buttons', 'fontSize'])} />
                  <LengthField label="Horizontal padding" placeholder="22px" value={get(['buttons', 'paddingX'])} onChange={set(['buttons', 'paddingX'])} />
                  <LengthField label="Vertical padding" placeholder="15px" value={get(['buttons', 'paddingY'])} onChange={set(['buttons', 'paddingY'])} />
                  <LengthField label="Letter spacing" placeholder="0.08em" value={get(['buttons', 'letterSpacing'])} onChange={set(['buttons', 'letterSpacing'])} />
                  <ChoiceField label="Text transform" value={get(['buttons', 'transform']) as never} options={TRANSFORM_OPTIONS} onChange={set(['buttons', 'transform'])} />
                </div>
              </Panel>
