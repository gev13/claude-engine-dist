...
type ListResponse = { items: Submission[]; total: number; page: number; perPage: number; forms: { name: string; n: number }[] };
            ...
            <dl className="m-0 grid gap-x-6 gap-y-2 sm:grid-cols-[minmax(0,220px)_minmax(0,1fr)]">
              {row.answers.map((answer) => (
                <Fragment key={answer.id}>
                  ...
                  <dd className="m-0 whitespace-pre-wrap text-[14px] text-ash">{answerText(answer.value) || '—'}</dd>
                </Fragment>
              ))}
            </dl>
          </article>
