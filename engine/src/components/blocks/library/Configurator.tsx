    ...
    <section className={cn('he-lsec he-cfg', TONES[p.tone ?? 'base'])}>
      <div className="shell">
        ...
        <div className="he-cfg__stage">
          ...
          {count > 1 && (
            <div className="he-arrows is-side">
              <button type="button" className="he-arrow" aria-label="Previous colour" onClick={() => step(-1)}>
                <Icon.Chevron dir="left" size={20} />
              </button>
              <button type="button" className="he-arrow" aria-label="Next colour" onClick={() => step(1)}>
        ...
        <div className="he-cfg__foot">
          <div role="radiogroup" aria-label="Colour" className="he-cfg__swatches" onKeyDown={arrowKeys(count, active, setActive)}>
            {p.options.map((o, i) => (
              <button
                key={o.name + i}
                type="button"
