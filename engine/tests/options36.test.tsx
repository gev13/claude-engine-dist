...
describe('a section’s eyebrow and card titles', () => {
  ...
  it('style the eyebrow, its marker, and the titles under the heading', () => {
    const out = blockStyleToCss('s1', blockStyleSchema.parse({ typography: { eyebrow: { color: '#ff412e', sizeMobile: '13px' }, subheading: { color: '#17bde7' } } }));
    expect(out).toContain('.he-b-s1 .he-eyebrow .type-eyebrow{color:#ff412e}');
    expect(out).toContain('.he-b-s1 .he-eyebrow__rule{background-color:#ff412e}');
    expect(out).toContain('.he-b-s1 :is(h3,h4,h5,h6){color:#17bde7}');
    expect(out).toMatch(/@media \(max-width:\d+px\)\{[^}]*\.he-b-s1 \.he-eyebrow \.type-eyebrow\{font-size:13px\}/);
  });
});
...
describe('lists of cards behind a slash', () => {
  const lists = [{ title: 'Covers', accent: '#ff412e', items: ['Network', 'Cloud'] }, { title: 'You get', items: ['A report'] }];
  ...
  it('put the slash inside each entry’s text, in the list’s colour', () => {
    ...
    expect(html).toContain('--he-ilist-accent:#ff412e');
